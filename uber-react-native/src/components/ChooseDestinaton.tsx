...
interface ChooseDestinationProps {
    ...
    onCoordinatesSelect: (origin: string, destination: string) => void;
  }

export default function ChooseDestination({ setSwitchHud, onCoordinatesSelect }: ChooseDestinationProps) {
    const [originStreet, setOriginStreet] = useState("");
    const [destinationStreet, setDestinationStreet] = useState("");
    const [originCoordinates, setOriginCoordinates] = useState<{ lat: number; lng: number } | null>(
        null
      );
      const [destinationCoordinates, setDestinationCoordinates] = useState<{ lat: number; lng: number } | null>(
        null
      );

      const onTraceRoute = async () => {
        const originCoords = await fetchCoordinates(originStreet);
        const destinationCoords = await fetchCoordinates(destinationStreet);
        ...
        <View style={styles.container}>
            ...
            <View style={styles.secondContainer}>
                ...
                <View style={styles.secondInput}>
                    <AutocompleteInputComponent
                        onChangeText={(text) => setDestinationStreet(text)}
                        onLocationSelect={(lat, lng) => {
